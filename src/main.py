...
CONFIG = {
    "livekit_url": os.getenv("LIVEKIT_URL", "wss://predator-gnzrgpca.livekit.cloud"),
...
def build_agent():
    ...
    return PredatorAgent(anthropic_api_key=CONFIG["anthropic_api_key"], openai_api_key=CONFIG["openai_api_key"])
...
async def dev_mode():
    ...
    ctx = builder.build(first_name="רחל", last_name="לוי", city="פתח תקווה", street="הרצל", house_number="12", registered_branch="פתח תקווה", support_score=0.62, campaign_type="primaries")
    session = agent.create_session("test_session_1", voter_context=ctx)
    print(f"\nSession created: persona={session.current_persona}, state={session.current_state.value}, support={session.support_score}")
    ...
    for i, voter_text in enumerate(voter_inputs, 1):
        ...
        if "error" in result:
            ...
            continue
        print(f"  State: {result['state']}, Resistance: {result['resistance']}, Persona: {result['persona']}")
        print(f"  Tactic: {result.get('tactic')}, TTS speed: {result['tts_params']['speed']}")
...
def main():
    ...
    if mode == "dev":
        ...
        from .battle_mode import main as battle_main
        battle_main()
